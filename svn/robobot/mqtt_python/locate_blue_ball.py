...
# Blue color range in HSV (Ottimizzato per la pallina azzurra 3D)
LOWER_BLUE = np.array([90, 50, 50])
UPPER_BLUE = np.array([130, 255, 255])

# Distance Thresholds (Area in pixels)
AREA_1 = 10000      # Estimated area at 20cm distance
AREA_2 = 14750       # Final target area at 15cm distance
CENTER_TOLERANCE = 30   
...
def loop():
    ...
    while not service.stop:
        ...
        width = frame.shape[1]
        img_center_x =(width // 2 ) + 15
        
        # Color processing
        hsv = cv.cvtColor(frame, cv.COLOR_BGR2HSV)
        mask = cv.inRange(hsv, LOWER_BLUE, UPPER_BLUE)
        
        # Pulizia della maschera (essenziale per eliminare i riflessi della plastica)
